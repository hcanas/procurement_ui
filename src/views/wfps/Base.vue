<script setup>
      ...
      },
      {
        ...
        valueGetter: params => {
          return new Intl.DateTimeFormat('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })
            .format(new Date(params.data.timeframe_from))
            + ' - '
            + new Intl.DateTimeFormat('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })
              .format(new Date(params.data.timeframe_to))
        },
      },
      {
        field: 'cost',
        sortable: true,
        width: 150,
        valueFormatter: params => new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(params.value),
        comparator: (num1, num2) => num1 - num2,
      },
      {
        headerName: 'Fund Source',
        field: 'fund_source.name',
        sortable: true,
        minWidth: 140,
        flex: 1,
        wrapText: true,
  ...
  const loadData = year => {
    ...
    axios.get(`${import.meta.env.VITE_API_URL}/wfps?year=${year}`)
      .then(response => {
        ag_grid.value.row_data = response.data.map(data => {
          data.fund_source.office = offices.value.find(office => office.id === data.fund_source.office_id);
          return data;
        });
      })
      .catch(() => {});
  ...
  const createdWfp = new_data => {
    new_data.fund_source.office = offices.value.find(office => office.id === new_data.fund_source.office_id);
    ag_grid.value.api.applyTransaction({ add: [new_data], addIndex: 0 });
    closeDrawer();
  };
  
  const updatedWfp = new_data => {
    new_data.fund_source.office = offices.value.find(office => office.id === new_data.fund_source.office_id);
